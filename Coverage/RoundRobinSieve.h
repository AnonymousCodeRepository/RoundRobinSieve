...
// double p_af=2.0/3.0;
 double p_af=1.0;
// double p_af = (-2.0 + k) / (2.0 * k) + 0.5 * sqrt((4.0 + k * k) / (k * k));
...
{
    int solution_num = 1;
    cout<<"RoundRobinSieve & Max node: "<<max_node<<endl;
    ...
    if (S[1].S_revenue > S_best.S_revenue) {
        S_best = S[1];
    }
    if (S_[0].S_revenue > S_best.S_revenue) {
        S_best = S_[0];
    }
    if (S_[1].S_revenue > S_best.S_revenue) {
        S_best = S_[1];
    }

    if (u_value > S_best.S_revenue) {

        S_best.replace_with_singleton(u_value, u_star);
    }

    Result final(S_best.S_revenue,S_best.S_cost,S_best.solution.size(),oracle_times,rounds);
